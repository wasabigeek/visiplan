...
describe('apply_monthly_updates()', () => {
  test('it calculates CPF OA correctly', () => {
    ...
    const person = new Person({ birthDate: new Date(1987, 0, 1), salarySchedule: new SimpleSalary({ startingSalary: 5000, startingYear: 2022 }) });

    const cpfSim = new CpfSalaryContributionSim({ accountStore, person });
  ...
  test("it stops increasing after a Person's retirement", () => {
    const accountStore = new AccountStore();
    const person = new Person({ birthDate: new Date(2000, 0, 1), retirementAge: 50, salarySchedule: new SimpleSalary({ startingSalary: 5000, startingYear: 2022 }) });
